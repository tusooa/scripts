...
my $excl_pre = qr/(?:语音翻译|[Tt][Mm]|(?:神)?(?:特(?:么|喵)|(?:他|她|它)妈)(?:的)?|所以(?:说)?|说来|话说(?:回来)?|说回来|然而|(?:可|但)(?:是)?|只是|因为|只因|十分|简直|并)/;
my $excl_post = qr/(?:呢|哉|${me}也|矣|$you|吧|了|[bp]u|的(?:样子|说)?|desu|极了|惹|呗)/;
my $excl_mid = qr/(?:)/;
# 动作和状态
my $status_line = qr/(?:摊手|捂脸|蹭(?:一下)?|(?:一脸)?[懵蒙]逼|划掉|划去|打勾|打钩|(?:并)?不|再见|(?:一脸)?生无可恋|大?[误雾]|抱[紧抱住]?|紧紧抱住|(?:摸{1,2}|摸一?下子?)${head}?|揉揉(?:$head|脸)?|举高高|大?笑|哭|埋胸)/;
...
our $replacements = {
    ...
    '中' => $emotion_mid,
    '我' => $me,
    '为什么' => qr/(?:为(?:什么|毛|喵|咩|何|啥子?)|怎么?(?:能够?|可以)?|咋)/,
    '你' => $you,
