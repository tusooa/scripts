...
# 这只是几个例子，绝对不是兔嫂在卖萌，嗯。
# 丢失了一些。。。。QAQ
my $emotion = qr/(?:w+|[PpQq](?:.[PpQq])+[PpQq]*|0(?:.0)+|[Oo](?:.[Oo])+|x+|-(?:.-)+|=(?:.=)+|😂|h+|☆|\(\?ω\?=\))/;
# 最后一个是笑抽的表情，嗯。
my $emotion_s = qr/(?:\s+|？|。|\?|\.|~|～|,|，|!|！|\^|【|】|\[|\]|（|\(|\)|）|「|」|“|”)/;
my $excl = qr/(?:呜|哟|哦|喵|咩|呜|吗|[啊阿][咧勒]?|呀|哪|呐|嘛|咪|口意|噫|吁|嚱|嗯|恩|诶多?|欸|哎|唉|等|噗|铥(?:的)?|这|并|23{2,}(?:4*3*)*|6+|哈|蛤|呵|咳|科科|哼|唧|摊手|捂脸|蹭(?:一下)?)/;
my $excl_pre = qr/(?:喂|卧槽|woc|[Tt][Mm]|(?:神)?(?:特(?:么|喵)|(?:他|她|它)妈)(?:的)?|所以(?:说)?|说来|话说(?:回来)?|说回来|然而|(?:可|但)(?:是)?|只是|因为|只因|十分|简直)/;
my $excl_post = qr/(?:呢|哉|也|矣|你|(?:大)?误|(?:大)?雾|吧|了|打勾|打钩|(?:并)?不|[bp]u|再见|(?:一脸)?生无可恋|的(?:样子|说)?|desu|极了|划掉|划去)/;
